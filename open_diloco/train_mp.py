...
def get_model(config: Config) -> LlamaForCausalLM:
    ...
    return LlamaForCausalLM.from_pretrained(pretrained_model_name_or_path=config.path_model, config=config_model, resume_download=True)
...
def train(config: Config):
    ...
    if config.parallelism == "tp":
        ...
        if world_messenger_hv:
            full_model = create_full_model_copy(model, device="cpu")
        model, pp_stage = wrap_model_pp(model, num_stages, pp_stage_index, torch.device(f"cuda:{local_rank}"))
    ...
    if config.hv is not None:
        if config.hv.outer_optimizer_type == "adam":
            outer_optimizer = partial(torch.optim.Adam, lr=config.hv.outer_lr, betas=config.hv.outer_betas, eps=config.hv.outer_eps, weight_decay=0)
        ...
        if config.hv.outer_sign_update:
            ...
            if config.hv.hivemind_compression is None:
                ...
                log(f"Auto-setting hivemind_compression='sign1bit' for 1-bit communication (return_deltas=False)")
            elif config.hv.hivemind_compression != "sign1bit":
                log(f"WARNING: outer_sign_update=True but hivemind_compression='{config.hv.hivemind_compression}' (not 'sign1bit'). 1-bit packing disabled.")
    ...
    for step, batch in enumerate(iterable=train_dataloader, start=start_step * gradient_accumulation_steps):
        ...
        if not is_accumulating:
            ...
            if config.ckpt.interval is not None and real_step % config.ckpt.interval == 0:
                ...
                ckpt_save_path = os.path.join(config.ckpt.path, f"{CKPT_PREFIX}_{int(real_step)}")
                if config.hv:
                ...
                if local_rank == 0 and config.ckpt.topk is not None:
                    deleted = delete_old_checkpoints(config.ckpt.path, config.ckpt.topk)
                    if deleted:
